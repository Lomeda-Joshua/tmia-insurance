...
////////////////////////////// BUTTONS CLICKED /////////////////////////////////
  ...
  $(document).on("click","#btnsave",function(){
    ...
    function CheckUserName() {
      $.ajax(
        {
          url : window.dataRoutes.checkUserName,
          type: "POST",
...
/////////////////// ENABLED OR DISABLED /////////////////////
    function FormDisable(val) {
        $("#txtlname").focus();
        $("#txtlname").prop("disabled", val);
        $("#txtfname").prop("disabled", val);
        $("#txtmname").prop("disabled", val);
        $("#txtsname").prop("disabled", val);
        $("#txtdname").prop("disabled", val);
        $("#txtcontactno").prop("disabled", val);
        $("#txtemailadd").prop("disabled", val);
        $("#txtuname").prop("disabled", val);
        $("#txtpass").prop("disabled", val);
        $("#txtrpass").prop("disabled", val);

        if ($.fn.validator) {
            $(".form-horizontal").validator('reset');
        }

        // Role-specific fields using standard JS conditions
        if (window.userLevel === "ADMINISTRATOR") {
            $("#dppwdexpdate, #chk2fa").prop("disabled", val);
        }

        if (val === true) {
            $("#btnedit").show().removeAttr("hidden");
            $(".actionbtn").hide().attr("hidden", true);
        } else {
            $("#btnedit").hide();
